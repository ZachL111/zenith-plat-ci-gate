...
`zenith-plat-ci-gate` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for ci analysis with bounded scenario files, conflict explanations, and documented operating limits.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 2.
